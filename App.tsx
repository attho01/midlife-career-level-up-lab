...
function App() {
  ...
  const handleAnalysis = async (inputData: CareerInput) => {
    ...
    try {
      ...
      if (msg.includes("429")) {
        alert("오늘의 AI 사용량이 초과되었습니다. 잠시 후 다시 시도하거나 다른 API 키를 사용해주세요.");
      } else if (msg.includes("Safety")) {
        alert("입력하신 내용 중 민감한 정보가 포함되어 AI가 분석을 거부했습니다. 내용을 수정하여 다시 시도해주세요.");
      } else {
  ...
  const handleJobSelect = async (jobName: string) => {
    ...
    try {
      ...
      const msg = error.message || "";
      if (msg.includes("429")) {
        alert("AI 사용량이 초과되었습니다 (Quota Exceeded). 잠시 후 다시 시도해주세요.");
      } else {
        alert(`모델 생성 중 오류가 발생했습니다: ${msg || '알 수 없는 오류'}`);
      }
    } finally {
      setLoading(false);
    }
  ...
  const handleModelSelect = async (modelType: string, duration: number) => {
    setLoading(true);
    setLoadingMessage(`매직 브리핑 생성 중 (${duration}개월)...`);
    ...
    try {
      ...
      const msg = error.message || "";
      if (msg.includes("429")) {
        alert("AI 사용량이 초과되었습니다. 무료 티어 한계일 수 있으니 잠시 후 다시 시도해주세요.");
      } else if (msg.includes("Safety") || msg.includes("차단")) {
        alert("취업 전략 생성 중 AI 안전 필터에 의해 내용이 차단되었습니다. 입력한 경력 데이터를 조금 더 일반적인 용어로 수정해보세요.");
      } else {
        alert(`전략 수립 중 오류가 발생했습니다: ${msg || '알 수 없는 오류'}`);
      }
    } finally {
      setLoading(false);
    }
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto p-4 md:p-8 relative">
          <div className="max-w-7xl mx-auto min-h-full flex flex-col">
            ...
            {/* Footer Copyright */}
            <footer className="mt-12 py-8 border-t border-slate-100 text-center no-print flex flex-col items-center gap-2">
              <p className="text-[10px] font-bold text-slate-300 uppercase tracking-[0.2em]">
                © 2026 ACLPro. All rights reserved
              </p>
              <p className="text-[8px] text-slate-200 font-medium">Build Ver: 20260505-V3</p>
            </footer>
          </div>
        </main>
      </div>
